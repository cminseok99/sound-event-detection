...
class CRNN(nn.Module):
    ...
    def __init__(self, n_in_channel, nclass, attention=False, activation="Relu", dropout=0,
        ...
        self.att = ATT(n_in_cnn, activation, dropout, **kwargs)
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        self.att.load_state_dict(state_dict['att'])
    ...
    def state_dict(self, destination=None, prefix='', keep_vars=False):
        state_dict = {"cnn": self.cnn.state_dict(destination=destination, prefix=prefix, keep_vars=keep_vars),
                      "att": self.att.state_dict(destination=destination, prefix=prefix, keep_vars=keep_vars),
    ...
    def save(self, filename):
        parameters = {'cnn': self.cnn.state_dict(), 'att': self.att.state_dict(), 'rnn': self.rnn.state_dict(), 'dense': self.dense.state_dict()}
    ...
    def forward(self,x):
        ...
        bs, chan, frames, freq = x_conv.size()

        if self.cnn_integration:
            x_conv = x_conv.reshape(bs_in, chan * nc_in, frames, freq)

        gate = self.att(torch.mean(ori,3)) # bs, chan, frames(628->157)
        
        if freq != 1:
            warnings.warn(f"Output shape is: {(bs, frames, chan * freq)}, from {freq} staying freq")
            x_conv = x_conv.permute(0, 2, 1, 3)
            x_conv = x_conv.contiguous().view(bs, frames, chan * freq)
        else:
            x_conv = x_conv.squeeze(-1)
            x_conv = x_conv*gate
            x_conv = x_conv.permute(0, 2, 1)  # [bs, frames, chan]
        # rnn features
        x = self.rnn(x_conv)
        x = self.dropout(x)
        ...
        if self.attention:
            ...
            weak = strong.mean(1)
        return strong, weak, x_conv, gate
