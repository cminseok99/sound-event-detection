...
def train(train_loader, model, optimizer, c_epoch, ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    for i, ((batch_input, ema_batch_input), target) in enumerate(train_loader):
        ...
        q, k, v = model(batch_input)
        
        
        
        strong_pred_ema, weak_pred_ema, conv_ema, _ = ema_model(ema_batch_input,q, k, v)
        strong_pred_ema = strong_pred_ema.detach()
        weak_pred_ema = weak_pred_ema.detach()
        conv_ema = conv_ema.detach()
        strong_pred, weak_pred, _, gate = model(batch_input)
        ...
        if ema_model is not None:
            ...
            # gate loss compared to EMA CNN output
            temp = conv_ema.sum(2).squeeze()
            maxV = torch.max(temp, 0)
            minV = torch.min(temp, 0)
            gate_label = (temp-minV.values) / (maxV.values-minV.values)

            gate_loss = class_criterion(gate.squeeze(), gate_label)
            meters.update('gate_loss', gate_loss.item())
        ...
        # 20240412 seminar
        np.save('batch_input.npy', batch_input.detach().cpu().numpy())
        np.save('ema_batch_input.npy', ema_batch_input.detach().cpu().numpy())
        np.save('target.npy', target.detach().cpu().numpy())

        np.save('strong_pred.npy', strong_pred.detach().cpu().numpy())
        np.save('weak_pred.npy', weak_pred.detach().cpu().numpy())
