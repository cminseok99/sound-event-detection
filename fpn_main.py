...
def train(train_loader, model, optimizer, c_epoch, ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    for i, ((batch_input, ema_batch_input), target) in enumerate(train_loader):
        ...
        if mask_strong is not None:
            strong_class_loss = class_criterion(strong_pred[mask_strong], target[mask_strong,:-1,:])
            meters.update('Strong loss', strong_class_loss.item())

            strong_ema_class_loss = class_criterion(strong_pred_ema[mask_strong], target[mask_strong,:-1,:])
...
if __name__ == '__main__':
    ...
    crnn_kwargs = {"n_in_channel": n_channel, "nclass": len(cfg.classes), "attention": True, "n_RNN_cell": 128,
                   ...
                   "fpn_cnn_kernel_size":3,
                   
                   "fpn_rnn_dropout_recurrent":0.5,
                   "fpn_cnn_dropout":0.5,
                   "fpn_rnn_n_RNN_cell":128,
                   "fpn_n_layers_RNN":2,
                   "fpn_rnn_type": "BGRU",
                   "fpn_rnn_dropout":0.5,
                   "scale_factor":[2,1],
                   "fpn_block_num" :2,
                   "upsample_type" :"cat",
                   "fpn_point_wise_chan" :1,
                   "fpn_point_wise_dropout":0.5,
